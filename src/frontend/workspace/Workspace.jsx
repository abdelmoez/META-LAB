...
import { SOURCE_OPTIONS, DATA_NATURE, ADJUST_OPTIONS, EXTRACT_FLAGS, DATA_NATURE_LABEL, ADJUST_LABEL, FLAG_LABEL, SOURCE_LABEL, ES_TYPES, ROB2, NOS, PRISMA_CL, MESH_DBS, PROSP_FIELDS, GRADE_DOMAINS } from "../../research-engine/project-model/monolithConstants.js";
import { buildMethodsMarkdown as jsMethodsMarkdown } from "../../research-engine/docs/methodsText.js";
// prompt44 item 2 — R validation engine (pure metafor-script generator; no execution).
import { buildMetaValidationR } from "../../research-engine/r-validation/rValidation.js";
import { fmtNum, fmtES, fmtCI, fmtEstCI, fmtP, fmtPct, fmtI2, fmtWeight, fmtInt, normalizePrecision, DECIMAL_OPTIONS } from "../../research-engine/format/precision.js";
...
import { api } from "../api-client/apiClient.js"; // prompt32 Task 10 — owner project delete from Project Control
...
import { openExportDialog, registerExportDialog } from "./exportDialogBridge.js";
...
import { SwitchToggle, SectionHeader, InfoBox, HelpTip, AIButton, ProgressBar, CriteriaList, Frac, MATH_FONT } from "./ui/primitives.jsx";
...
import { AI_FEATURES_ENABLED, CLAUDE_MODELS, callClaude, callClaudeWeb, fetchCitationAI, fileToBase64, fetchByDOI, fetchByPMID, testClaudeConnection, safeParseJSON, parseSections, parseBullets, parseTermReasons, parseConceptBlocks, parseFilters } from "../services/aiService.js";
...
/* ScreeningModule, MetaSiftPrismaSync, PRISMATab, PrismaFigureExport extracted
   ...
   and imported below. */
import { ScreeningModule, MetaSiftPrismaSync, PRISMATab, PrismaFigureExport } from "./tabs/screeningTabs.jsx";
...
import { AnalysisTab, DataBehindAnalysis, ResearchExport, ResultsWriteup, ForestTab, SensitivityTab, SubgroupTab } from "./tabs/analysisTabs.jsx";

/* ════════════ TABS: REPORTING / GRADE / MANUSCRIPT / METHODS (extracted prompt46 Phase 6h — verbatim) ════════════
   ReportTab, GRADETab, ManuscriptTab, MethodsTab moved VERBATIM to
   src/frontend/workspace/tabs/reportTabs.jsx and imported below. GRADE_OPTIONS /
   gradeSuggestions (used by GRADETab) live in projectHelpers.js; Frac/MATH_FONT
   in ui/primitives.jsx — both imported there. */
import { ReportTab, GRADETab, ManuscriptTab, MethodsTab } from "./tabs/reportTabs.jsx";

/* ════════════ TAB: REPORTING CHECKLIST ════════════ */
/* ReportTab extracted VERBATIM to src/frontend/workspace/tabs/reportTabs.jsx
   (prompt46 Phase 6h) and imported below. */
...
/* GRADETab extracted VERBATIM to src/frontend/workspace/tabs/reportTabs.jsx
   (prompt46 Phase 6h) and imported below. */

/* ════════════ TAB: AI MANUSCRIPT DRAFTER ════════════ */
/* ManuscriptTab extracted VERBATIM to src/frontend/workspace/tabs/reportTabs.jsx
   (prompt46 Phase 6h) and imported below. */

/* ════════════ METHODS & EQUATIONS TAB (prompt6 Task 13) ════════════ */
/* MethodsTab extracted VERBATIM to src/frontend/workspace/tabs/reportTabs.jsx
   (prompt46 Phase 6h) and imported below. */
...
export default function MetaLab({ initialProjectId = null, initialTab = null, onProjectChange = null, onTabChange = null, onBackToProjects = null } = {}){
  ...
  const buildReportHTML=(precOverride)=>{
    ...
    <h1>${esc(p.name)}</h1>
    <div class="muted">Systematic review &amp; meta-analysis report · generated ${new Date().toLocaleDateString()} · PecanRev</div>
    ...
    <div class="muted" style="margin-top:30px;border-top:1px solid #ccc;padding-top:8px;">Generated by PecanRev. Verify all numbers against your primary analysis before submission. Statistical methods: inverse-variance fixed effect and DerSimonian–Laird random effects${res&&res.hksj?", with Hartung–Knapp–Sidik–Jonkman adjustment":""}.</div>
    </body></html>`;
    return html;
  };
  ...
  if(loading) return(
    <div style={{background:C.bg,color:C.txt,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Sans',sans-serif"}}>
      <div style={{textAlign:"center"}}>
        ...
        <div style={{fontSize:15,fontWeight:700,color:C.txt,marginBottom:6}}>PecanRev</div>
  ...
  return(<div style={{display:"flex",minHeight:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans',sans-serif",color:C.txt}}>
    ...
    {confirmDel&&(()=>{
      ...
      const linkedTitle=delProj._linkedMetaSift&&delProj._linkedMetaSift.id
        ?(delProj._linkedMetaSift.title||"linked Screening workspace"):null;
      ...
      return(<div className="modal-bg" style={{position:"fixed",inset:0,background:"#00000099",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
        <div style={{
          ...
          <ul style={{margin:"0 0 16px 18px",padding:0,fontSize:12,color:C.muted,lineHeight:1.75}}>
            <li>The PecanRev project — studies, extraction data, analyses, and figures</li>
    ...
    <div className="ml-sidebar" style={{
      ...
      <div style={{padding:"18px 16px 14px",borderBottom:`1px solid ${C.brd}`}}>
        <div style={{display:"flex",alignItems:"center",gap:9}}>
          ...
          <div>
            <div style={{fontSize:14,fontWeight:800,color:C.txt,letterSpacing:-0.2,lineHeight:1}}>PecanRev</div>
