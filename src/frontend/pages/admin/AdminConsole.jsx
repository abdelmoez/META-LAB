...
// 95.md — the schema-driven user edit form + registration-country lookup moved
// into the extracted users/ package (UserDrawer); only COUNTRY_OPTIONS is still
// consumed here (the Beta Waitlist country filter).
...
// 95.md — the redesigned Ops user directory lives in its own package
// (src/frontend/pages/admin/users/). It owns the table, server-side filters,
  ...
  return (
    <div>
      ...
      <OpsErrorBoundary key={view} label={`the ${view} view`}>
        {view === 'directory'    && (
          <>
            {/* Country choropleth stays admin-only above the directory (preserved
                from the pre-95 layout — the new container owns the table below). */}
            {isAdmin && <UsersByCountryCard />}
            <UsersDirectory isAdmin={isAdmin} />
          </>
        )}
