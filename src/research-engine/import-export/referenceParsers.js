...
/* Pure string→object reference-import parsers extracted verbatim from the
   monolith (meta-lab-3-patched.jsx). No React / no `C` / no Icon / no JSX. */

const uid = () => Math.random().toString(36).slice(2, 10);

export function isNonPrimary(s){
...
export function normTitle(t){ return String(t||"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim().replace(/\s+/g," "); }
...
export function parseReferences(text,filename){
