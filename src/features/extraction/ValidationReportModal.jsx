    ...
    <div style={{ position: 'fixed', inset: 0, background: '#00000099', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: C.surf, border: `1px solid ${C.brd}`, borderRadius: 12, padding: 20, width: '100%', maxWidth: 680, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14, gap: 12 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: C.txt }}>AI accuracy report</div>
            <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>How AI suggestions compared to the human consensus (gold standard) in this project.</div>
          </div>
        ...
        {state === 'ready' && empty && (
          <div style={{ fontSize: 12.5, color: C.muted, lineHeight: 1.6, padding: '8px 2px' }}>
            {report.note || 'No studies have both AI suggestions and human consensus values yet — the report needs a human gold standard first.'}
          </div>
        )}
        ...
        {state === 'ready' && !empty && (
          <>
            ...
            <div style={{ fontSize: 10.5, color: C.dim, marginTop: 10, lineHeight: 1.5, background: themeAlpha(C.yel, '0d'), borderRadius: 6, padding: '8px 10px' }}>
              These rates describe the AI assistant's agreement with human decisions — they are a quality signal, not a licence to skip review. Every AI suggestion still requires human acceptance.
            </div>
          </>
        )}
      </div>
