...
export async function buildManuscriptDocx(project, draft, opts = {}) {
  ...
  if (refList.length) {
    for (const r of refList) {
      ...
      // 118.md §27 — a hanging indent so a wrapped entry's continuation lines align
      // under the text rather than under the number, which is what makes a numbered
      // bibliography readable in Word and what a journal template expects to restyle.
      // PARAGRAPH-LEVEL ONLY: the runs above are the rendered citation-style output
      // (referenceLibrary117 / harvardDisambiguation117 pin their text and their
      // italic/bold segmentation), so nothing about what the entry SAYS changes here.
  ...
  const doc = new Document({
    ...
    //
    // 118.md §22/§23 — NEUTRAL, UNBRANDED WORD OUTPUT.
    //
    // Everything this exporter writes itself was already black-on-white; the ONLY
    // colour in the finished manuscript came from the docx library's own defaults.
    // `styles.default` is handed straight to its DefaultStylesFactory, which merges
    // per-style overrides over `{run:{color:'2E74B5'|'1F4D78', size:…}}` — so leaving
    // these keys unset shipped Word-blue Heading 1/2/4 (2E74B5), darker-blue Heading 3
    // (1F4D78) and an italic Heading 4 into every exported manuscript. That is exactly
    // the "application styling in the .docx" §22 forbids, and it is invisible from
    // document.xml (the colour lives in word/styles.xml).
    //
    // The merge is a SHALLOW spread over `run`, so each override must restate the size
    // it wants to keep: §24 asks for a real heading hierarchy, not a flattened one, so
    // Word's own 16/13/12pt steps are re-declared here rather than silently dropped.
    // The Hyperlink character style keeps the library's Word-blue 0563C1 — that is
    // Word's own convention for an external link, not our branding (§23), and the
    // plain-black InternalHyperlink cross-references are handled at the run level in
    // parseInline.
    styles: {
      default: {
        document: { run: { font: 'Calibri', size: 22 } },
        title: { run: { size: 56, color: '000000' } },
        heading1: { run: { color: '000000', size: 32 } },
        heading2: { run: { color: '000000', size: 26 } },
        heading3: { run: { color: '000000', size: 24 } },
        heading4: { run: { color: '000000', italics: false } },
        // The factory emits Heading 5/6 unconditionally, whether or not this document
        // uses them (markdownToParagraphs stops at H4). Their blue would still sit in
        // word/styles.xml for a journal template — or the researcher — to inherit, so
        // they are neutralised too; that is what makes "no 2E74B5/1F4D78 anywhere in
        // styles.xml" a checkable property rather than a claim about our own headings.
