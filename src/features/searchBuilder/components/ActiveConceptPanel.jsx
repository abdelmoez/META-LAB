...
export default function ActiveConceptPanel({ concept, conceptIndex, status, readOnly, onRename, onRenameCommit, onUpdateSourcePhrase, onRequestSplit, children }) {
  const c = concept || {};
  ...
  return (
    <section data-testid="sb-active-concept" aria-label={`Search group: ${c.label || ''}`}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
        {/* 97.md Phase 8 — neutral ordinal: the header always names its position in
            the AND chain ("Search Group N"), independent of any custom name. */}
        <span data-testid="sb-group-ordinal" title="This group's position in the AND chain"
          ...
          Group {(Number.isInteger(conceptIndex) ? conceptIndex : 0) + 1}
        </span>
        <input value={c.label || ''} onChange={(e) => { if (!readOnly && onRename) onRename(e.target.value); }}
          ...
          title={readOnly ? 'Read-only access — ask a project editor to rename this group' : 'Rename this search group'}
        ...
        {/* (97.md — the "Legacy group" badge is retired; legacy picoField groups render
            like any other search group. `picoField` itself is RETAINED in state.) */}
        {c.picoField !== 'T' && live.length > 0 && (
          <span data-testid="sb-mesh-coverage"
            title={meshN > 0 ? 'This group includes a matched MeSH term' : 'No MeSH term yet — adding one from the suggestions usually improves recall'}
      ...
      <p style={{ margin: '0 0 12px', fontSize: 12, color: C.muted, lineHeight: 1.6 }}>
        Add different words researchers may use for {c.label || 'this idea'}. Terms in this group are combined with <strong>OR</strong> — any one of them counts as a match. Separate groups are combined with <strong>AND</strong>.
