...
export const AUDIT_ACTIONS = {
  ...
  // ── 112.md follow-up — Ops › Email template management ─────────────────────
  UPDATE_EMAIL_TEMPLATE: { severity: 'medium', category: 'content', label: 'Edited email template',
    describe: (d, log) => `edited the "${log?.entityId || 'unknown'}" email template copy${fieldList(d) ? ` (${fieldList(d)})` : ''}` },
  RESTORE_EMAIL_TEMPLATE: { severity: 'medium', category: 'content', label: 'Restored email template',
    describe: (d, log) => `restored the "${log?.entityId || 'unknown'}" email template to its default copy` },
  EMAIL_TEMPLATE_TOGGLE: { severity: 'medium', category: 'content', label: 'Toggled email template',
    describe: (d, log) => `${d.enabled === true ? 're-enabled' : 'DISABLED'} the "${log?.entityId || 'unknown'}" email template` },
  EMAIL_TEST_SEND: { severity: 'info', category: 'content', label: 'Sent test email',
    describe: (d, log) => `sent themselves a test "${log?.entityId || 'unknown'}" email${d.recipient ? ` (${d.recipient})` : ''}` },
