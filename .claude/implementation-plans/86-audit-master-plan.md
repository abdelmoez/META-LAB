...
- **Phase B — The blob-writer concurrency phase (isolated).** P0.2 + P1.14/15/16/18/21 + P2.60/88: introduce `store.mutateProjectBlob`, route all writers through it, subscribe Stitch doc to realtime, add CAS to `updateProject`. High blast radius → own PR + integration tests + manual multi-tab verification.
- **Phase C — Statistics consumer unification.** P1.6, P2.16/17/18/19 (kill the duplicate engine copy or add cross-copy parity tests; route all pooled views through outcome grouping; thread estimator into exports/R script).
- **Phase D — Scalability.** Move O(n²) dedup + duplicate detection to the worker pool (P2.96); paginate/stream `listRecords`, `getMetrics`, full-text coverage (P2.42/90/91/102); add missing indexes (P2.56); batch N+1s (P3.14).
- **Phase E — AuthZ/privacy centralization.** `projectRecordForViewer` projection (P1.1 long-term, P3.104); enforce `canRunAnalysis` server-side (P3.1); enforce or de-advertise unenforced entitlements (P2.3); AI-score blind gating (P2.93); session revocation on role change (P2.2/34).
- **Phase F — CI/observability/deploy hardening.** Integration CI job (P1.4); lint + typecheck + coverage (P3.22); version-control the deploy script and encode schema-to-prod (P2.11, P3.69); release-id injection (P3.34).
- **Phase G — UX & docs.** Confirmations on destructive RoB/extraction actions (P3.56/81); pagination-stable teammate decisions (P2.100); deep-link preservation after login (P2.24); stale-comment sweep (P3.10/24/53/92).
